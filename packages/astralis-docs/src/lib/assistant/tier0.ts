...
export const TIER0: Tier0Entry[] = [
  {
    ...
    answer: `**Astralis UI** is a React 19 component library built on semantic design tokens — **75 components** that ship as precompiled CSS with zero build tooling required in your app.
...
It also coexists with whatever styling setup you already have (utility frameworks, CSS Modules, plain CSS): every internal class is namespaced under an \`astralis:\` prefix, so nothing can collide, and your own classes work in \`className\` on any component. Details: [Installation](/docs/installation).`,
  },
  {
    ...
    answer: `**75 components across 9 categories** — buttons, layout, typography, data entry (Input → Combobox), overlays (Modal, Drawer, Popover, Tooltip, Menu), data display (Table, Calendar, Avatar…), feedback (Toast, Alert, Progress, Skeleton, Spinner), navigation and disclosure.
...
The stylesheet is ~120 KB gzipped (all 75 components, both themes, all responsive variants — imported once). Runtime dependencies: four small ones; no Radix, no Floating UI, no animation library.`,
  },
  {
    id: "toast",
    aliases: [
