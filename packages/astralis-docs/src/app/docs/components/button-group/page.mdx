...
direct child [Button](/docs/components/button) inherits it — the group hands
the props down during server render, so the sharing costs no client
JavaScript. An explicit prop on a child always wins, which is how the
